...
func _on_jumpscare_body_entered(body: Node3D) -> void:
	if body == player_node:
		if GlobalNode.follow == 1:
			Jumpscare.play()
			GlobalNode.canMove = 0
			GlobalNode.follow = 0
			Player.look_at(self.global_position, Vector3.UP)


func hear_noise(noise_position: Vector3, hearing_range: float) -> void:
	var distance_to_noise = global_position.distance_to(noise_position)
	if distance_to_noise <= hearing_range and GlobalNode.follow == 1:
		nav_agent.target_position = player_node.global_position
